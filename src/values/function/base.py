...
class BaseFunction(Value):
    ...
    def check_args(self, arg_names, args):
        ...
        got = len(args)
        signature = f"({', '.join(arg_names)})" if arg_names else "()"

        if got > expected:
            return res.failure(RTError(
                self.pos_start, self.pos_end,
                f"'{self.name}'{signature} takes {expected} argument{'s' if expected != 1 else ''}, but {got} were given",
                self.context
            ))
        ...
        if got < expected:
            ...
            return res.failure(RTError(
                self.pos_start, self.pos_end,
                f"'{self.name}'{signature} missing argument{'s' if len(missing) != 1 else ''}: {missing_str}",
                self.context
            ))
    ...
    def resolve_args(self, arg_names, defaults, args, kwargs, exec_ctx):
        ...
        if len(args) > n:
            return res.failure(RTError(
                self.pos_start, self.pos_end,
                f"'{self.name}'({', '.join(arg_names)}) takes {n} argument{'s' if n != 1 else ''}, but {len(args)} were given",
                self.context
            ))
        ...
        for kw_name, kw_val in kwargs.items():
            if kw_name not in arg_names:
                return res.failure(RTError(
                    self.pos_start, self.pos_end,
                    f"'{self.name}' got unexpected keyword argument '{kw_name}'. Expected one of: {', '.join(arg_names)}",
                    self.context
                ))
        ...
        for i, name in enumerate(arg_names):
            if name not in filled:
                if defaults[i] is not None:
                    ...
                    return res.failure(RTError(
                        self.pos_start, self.pos_end,
                        f"'{self.name}' is missing required argument '{name}'",
                        self.context
                    ))
